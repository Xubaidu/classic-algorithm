...
int128 INV_RSA(const string &cipher, const PBK& pbk, const PRK &prk);
void showInfo(const string &plaintext, const int128 &plain, const PBK &pbk, const PRK &prk, const int128 &cipher, const string &message);
...
string dec2bin(const int128 &num)
{
    ...
    cnt += 1;
    for (int i = 0; i < cnt - bin.length(); ++i) bin += '0';
...
int128 RSA(const int128 &plain, const PBK &pbk)
{
    int128 cipher = qpow(plain, pbk.e, pbk.n);
    return cipher;
}

string INV_RSA(const string &cipher, const PBK& pbk, const PRK &prk)
{
    int128 message = qpow(cipher, prk.d, pbk.n);
    string ret = num2plain(message);
    return ret;
}
...
void solve(const string &plaintext, const int &key_length)
{
    ...
    int128 plain = plain2num(plaintext);
    int128 cipher = RSA(plain, pbk);
    string message = INV_RSA(cipher, pbk, prk);
    showInfo(plaintext, plain, pbk, prk, cipher, message);
}
